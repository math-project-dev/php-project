				...
				if ($selectType == 1)
				{
					...
					{ ?>
						<div class="tasks animated fadeInDown">
							...
							<img class="task" src="../ege/images/type-<?=$selectType?>/0<?=$selectTask?>/0<?=$selectTask?>_0<?=$i?>.png" />
							<div class="answerDiv">
							<div>
								<script>
							...
							<div class="answerImage" >
								<img id="answer-<?=$i?>" style="display: none; padding: 5px" src="../ege/images/type-<?=$selectType?>/answer/0<?=$selectTask?>/0<?=$selectTask?>_0<?=$i?>.png" />
								<div>
									<? $result = mysql_query('SELECT answer FROM answers WHERE id='. $i .' AND tasks='. $selectTask .' AND type='. $selectType .'  '); ?>
